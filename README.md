...
More private than running an agent: no resident process, no credentials to leak, **no sandbox VM needed** (OpenClaw · NemoClaw · Hermes · OpenShell all require one) — Shortcuts runs on-device behind Apple's permission prompts, so the OS *is* the sandbox. See [Why Shortcuts?](docs/why-shortcuts.md) for the full privacy/security case.
